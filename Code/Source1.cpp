		...
		if (roll.size() == 6) {
			...
			if (studentrec_file) {
				...
				if (search == true) {
					...
					file_printer(y = 2);
					SetConsoleTextAttribute(h, 11);
					cout << "\n#" << setfill('#') << setw(150) << "#\n" << setfill(' ') << setw(45) << "Transcript of student\n\n" << setw(18) << "subject" << setw(19) << "credit hours" << setw(17) << "grade" << endl;
					find_transcript();
...
void Data_entry() {
	...
	if (name.size() < 14)
		name.insert(name.size(), 14 - name.size(), '?');
	...
	if (fname.size() < 14)
		fname.insert(fname.size(), 14 - fname.size(), '?');
	...
	if (address.size() < 28)
		address.insert(address.size(), 28 - address.size(), '?');
	dob = "";
	while (dob.size() != 8) {
...
void Store_grades() {
	...
	for (int i = 0; i < n; i++) {
		...
		string u;
		u = to_string(subject[i].credit_hours);
		u[0] = u[0] + 100;
		if (u.size() > 1)
			u.replace(1, u.size(), "");
		Grade_rec.open("Transcript_file", ios::app);
		Grade_rec << ";" << E_subject << "-" << u << "-" << E_grade;
		Grade_rec.close();
	}
	Grade_rec.open("Transcript_file", ios::app);
	Grade_rec << ":";
